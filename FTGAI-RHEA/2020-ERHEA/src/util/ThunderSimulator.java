...
public class ThunderSimulator {
	 ...
	 *
	 * @param gameData an instance of the GameData class
	 */
	public ThunderSimulator(GameData gameData) {
		this.gameData = gameData;
	}

	/**
	 * Simulates the progression of a fight starting from a given {@link FrameData}
	 * instance and executing specified actions for both players.<br>
	 * The resulting {@link FrameData} is returned, and can be used to assess the
	 * outcome of the simulation.<br>
	 *
	 * Note that when the character is on ground, all AIR actions will be considered
	 * invalid by the simulator. Likewise, all GROUND actions are considered invalid
	 * if the character is in air. <br>
	 ...
	 * @param playerNumber    boolean value which identifies P1/P2. {@code true} if
	 *                        the player is P1, or {@code false} if P2.
	 * @param myAct           actions to be performed by the player identified by
	 *                        the parameter `player`
	 * @param oppAct          actions to be performed by the opponent of the player
	 ...
	 *                        required for executing the actions of a given player,
	 *                        the simulator will simulate the player's actions up to
	 *                        `simulationLimit` frames; otherwise, the simulator
	 *                        will simulate the player's all actions and then
	 *                        continue the simulation until the `simulationLimit`-th
	 *                        frame assuming no actions are performed by the player.
	 *
	...
	public FrameData simulate(FrameData frameData, boolean playerNumber, Deque<Action> myAct, Deque<Action> oppAct,
		...
		// ArrayList<Motion> p1MotionData = this.gameData.getMotion(playerNumber ? true
		// : false);
		// ArrayList<Motion> p2MotionData = this.gameData.getMotion(!playerNumber ? true
		// : false);
		...
		ThunderSimFighting simFighting = new ThunderSimFighting();
