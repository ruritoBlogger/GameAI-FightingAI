...
public class ParameterSet {
    ...
    public boolean isRMHC() {
        return POPULATION_SIZE == 1;
    }

    public boolean isInnerMacro() {
        return INNER_MACRO_ACTION_LENGTH > 1;
    }

    public String[] getParams() {
        String[] s = new String[] { "Population size", "Simulation depth", "Init type", "Budget type",
                "Budget (FM calls)", "Heuristic type", "Macro-action length", "Inner macro-action length",
                "Bandit mutation", "Softmax bias in mutation", "Diversity in mutation", "Crossover type",
                "Statistical tree", "Stat tree recommend", "Shift buffer", "Rollouts", "Rollout length",
                "Repeat rollout N", "Population diversity", "Diversity type", "Diversity weight",
                "Reevaluate individuals", "N genes mutated", "Tournament size", "N parents", "Resample rate", "Elitism",
                "Discount reward", "Discount shift buffer" };
        return s;
    }

    public String[] getDefaultValues() {
        String[] s = new String[] { "" + POPULATION_SIZE, "" + SIMULATION_DEPTH, "" + INIT_TYPE, "" + BUDGET_TYPE,
                "" + MAX_FM_CALLS, "" + HEURISTIC_TYPE, "" + MACRO_ACTION_LENGTH, "" + INNER_MACRO_ACTION_LENGTH,
                "" + BANDIT_MUTATION, "" + MUT_BIAS, "" + MUT_DIVERSITY, "" + CROSSOVER_TYPE, "" + TREE,
                "" + SHIFT_BUFFER, "" + ROLLOUTS, "" + ROLLOUT_LENGTH, "" + REPEAT_ROLLOUT, "" + POP_DIVERSITY,
                "" + DIVERSITY_TYPE, "" + D, "" + REEVALUATE, "" + MUTATION, "" + TOURNAMENT_SIZE, "" + NO_PARENTS,
                "" + RESAMPLE, "" + ELITISM, "" + DISCOUNT, "" + SHIFT_DISCOUNT };
        return s;
    }

    public Object[][] getValueOptions() {
        return new Object[][] { new Integer[0], new Integer[0], new String[] { "Random", "OneStep", "MCTS" },
                new String[] { "Full budget evo", "Budget include init" }, new Integer[0],
                new String[] { "WinScore", "SimpleState" }, new Integer[0], new Integer[0],
                new Boolean[] { false, true }, new Boolean[] { false, true }, new Boolean[] { false, true },
                new String[] { "1-Point", "Uniform" }, new Boolean[] { false, true }, new Boolean[] { false, true },
                new Boolean[] { false, true }, new Boolean[] { false, true }, new Integer[0], new Integer[0],
                new Boolean[] { false, true }, new String[] { "Genotypic", "Phenotypic" }, new Double[0],
                new Boolean[] { false, true }, new Integer[0], new Integer[0], new Integer[0], new Integer[0],
                new Integer[0], new Double[0], new Double[0] };
    }
    ...
    public String toString() {
        ...
        if (INIT_TYPE == INIT_RANDOM)
            init = "random";
        else if (INIT_TYPE == INIT_ONESTEP)
            init = "OneStep";
        else if (INIT_TYPE == INIT_MCTS)
            init = "MCTS";

        String bud = "none";
        if (BUDGET_TYPE == FULL_BUDGET)
            bud = "full budget";
        else if (BUDGET_TYPE == HALF_BUDGET)
            bud = "half budget";

        String heur = "none";
        if (HEURISTIC_TYPE == HEURISTIC_WINSCORE)
            heur = "WinScore";
        else if (HEURISTIC_TYPE == HEURISTIC_SIMPLESTATE)
            heur = "SimpleState";

        String cross = "none";
        if (CROSSOVER_TYPE == UNIFORM_CROSS)
            cross = "uniform";
        else if (CROSSOVER_TYPE == POINT1_CROSS)
            cross = "1-Point";
