...
public class BaseAI {
	...
	void setMyHittingAction() {
		...
		for (Action ac : myacts) {
			...
			if (hpScore > 0) {
				...
				double turnScore = (double) hpScore + (mo.attackDownProp ? 30.0 : 0.0)
	...
	public void getInformation(FrameData frameData) {
		...
		if (frameData.getFramesNumber() >= 0) {
			...
			if (oppmo.getAttackHitDamage() > 0)
				this.lastOppAttackMotion = oppmo;
	...
	public String getDoAction() {
		...
		if (this.c_name == CharName.ZEN) {
			...
			// v1.2----------------------------------------------------------------------------------------
			// Fighting huge beat!
			if (opp.getState().equals(State.DOWN) && (distance <= 190 && distance >= 50)
					&& calc.IsInHitArea(Action.STAND_D_DF_FC) && calc.canHitFromNow(Action.STAND_D_DF_FC, this.player)
					&& my.getEnergy() >= 150)
			...
			if (calc.getMinHpScore(Action.CROUCH_FB, opActs) > 0 && distance <= 180) {
				return Action.CROUCH_FB.name();
			}

			// ----------------------------------------------------------------------------------------------

			// // Fighting huge beat! -- win
			// RHEA_PI----------------------------------------------------------
			//
			// // Fighting huge beat!
			...
			//
			// // The amount of Energy larger than 200
			// if ( (opp.getState().equals(State.STAND) ||
			// opp.getState().equals(State.CROUCH)) &&
			// (distance >=20 && distance <= 200)
		...
		// Garnet---------------------------------------------------------------------
			...
			// knock down skill.
			LinkedList<Action> oppMove = calc.getEnoughEnergyActions(!this.player, Action.NEUTRAL, Action.STAND_D_DF_FA,
					Action.STAND_D_DF_FB);
			if (my.getState() == State.AIR && calc.getMinHpScore(Action.AIR_UB, oppMove) > 0) {
				last_AIR_UB = true;
				return Action.AIR_UB.name();
			}
			if (bestact != null && calc.getMinHpScore(bestact, oppMove) > 0 && last_AIR_UB)
				return bestact.name();
			else
				last_AIR_UB = false;
		}
		...
		if (myHittingActions.size() > 0)
			...
			// From Thunder
			LinkedList<Action> moveActs = (distance < this.distance_threshold)
					? calc.getEnoughEnergyActions(this.player, Action.FOR_JUMP, Action.FORWARD_WALK, Action.NEUTRAL,
							Action.JUMP, Action.BACK_JUMP, Action.BACK_STEP)
					: calc.getEnoughEnergyActions(this.player, Action.FORWARD_WALK, Action.FOR_JUMP, Action.NEUTRAL,
							Action.JUMP, Action.BACK_JUMP, Action.BACK_STEP);
